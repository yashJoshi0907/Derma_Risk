"""
test_gradcam2.py — Self-contained Grad-CAM test (from grad2.ipynb)
Change IMAGE_PATH below, then run: python test_gradcam2.py
"""
...
# ══════════════════════════════════════════════════════════════════════════════
#  2. PREPROCESS  (from grad2.ipynb — 180px center crop + preprocess_input)
# ══════════════════════════════════════════════════════════════════════════════

def crop_center(img: np.ndarray, size: int = 180) -> np.ndarray:
    h, w, _ = img.shape
    startx  = w // 2 - size // 2
    starty  = h // 2 - size // 2
...
def preprocess_image(img_path: str):
    ...
    img     = crop_center(img, size=180)           # 180px center crop
    raw_224 = cv2.resize(img, (224, 224))           # keep uint8 copy for overlay
    img_f   = preprocess_input(raw_224.astype(np.float32))
    img_arr = np.expand_dims(img_f, axis=0)

    print("✅ Image preprocessed")
    print(f"   img_array : {img_arr.shape}  {img_arr.dtype}")
    print(f"   raw_224   : {raw_224.shape}  {raw_224.dtype}")
    return img_arr, raw_224


# ══════════════════════════════════════════════════════════════════════════════
#  3. GRAD-CAM SPLIT MODELS
# ══════════════════════════════════════════════════════════════════════════════

def build_gradcam_models(model: tf.keras.Model):
    """
    Layer order (Input not counted in model.layers):
...
def make_gradcam(conv_model, classifier_model, img_array):
    """
    Returns
    -------
...
def overlay_gradcam(raw_224: np.ndarray, heatmap: np.ndarray, alpha: float = 0.6) -> np.ndarray:
    ...
    Parameters
    ----------
    raw_224 : (224, 224, 3) uint8 RGB — from preprocess_image()
    heatmap : (224, 224) float32
...
def plot_results(raw_224, heatmap, overlay_rgb, pred_class, confidence, risk, predictions):
    ...
    for bar, prob in zip(bars, probs):
        ax4.text(min(prob + 0.02, 0.92), bar.get_y() + bar.get_height() / 2,
    ...
    plt.tight_layout()
    plt.savefig("gradcam2_result.png", dpi=150, bbox_inches="tight")
    print("✅ Saved → gradcam2_result.png")
    plt.show()
...
if __name__ == "__main__":
    ...
    # 4. Run Grad-CAM
    heatmap, pred_idx, predictions = make_gradcam(conv_model, classifier_model, img_array)
